...
def get_args():
    ...
    parser.add_argument('--source_dir', type=str, required=True, help='Path to the input checkpoint directory containing ZeRo1 checkpoint shards, i.e. mp_rank_*_model_states.pt.')
    parser.add_argument('--glob-pattern', type=str, default='mp_rank_*_model_states.pt', required=False, help='Filename pattern to glob for ZeRo1 checkpoint shards.')
    parser.add_argument('--output_dir', type=str, required=True, help='Path to the output checkpoint directory to dump the --mp_size converted model checkpoint (ZeRo1).')
    parser.add_argument('--mp_size', type=int, required=True, help='Desired output model parallelism to convert to.')
    parser.add_argument('--exclude-extra', action='store_true', help='Exclude extra states in the conversion. Default to False, i.e. include extra states.')
    ...
    if partition_dim is None:
        for i, tensor in enumerate(tensors):
            if not torch.allclose(tensors[0], tensor):
                logging.info(f'WARNING: Synchronized params differ for param {tensor_name}: abs max diff = {(tensors[0] - tensor).abs().max()}.')
...
def split_tensor_across_shards(
    ...
    if partition_dim is None:
        ...
        if tensor.shape[partition_dim] % n_shards != 0:
            raise ValueError(f"Cannot shard {tensor_name} of dimension {tensor.shape[partition_dim]} across {n_shards} evenly.")
...
def check_params(detected, expected, buffers: set[str], param_pattern=DEFAULT_PARAM_PATTERN, verbose=False):
...
def convert(input_data_shards, output_data_shards, model_parameter_names: List[str], param_list: List[Param], verbose=False, exclude_extra=False):
    ...
    logging.info(f"Converting {len(model_parameter_names)} parameters from {len(input_data_shards)} input shards to {len(output_data_shards)} output shards...")
    converted = 0
    skipped = 0
        ...
        for param in param_list:
            if '.'.join(model_parameter.split('.')[2:]) == param.name:
                if param_info is None:
                    ...
                    raise ValueError(f'Found more than one matching model parallelism parameter for {model_parameter}: {param_info}, {param}')
        if param_info is None:
...
def convert_zero1_model_parallel_checkpoint(source_dir: str, output_dir: str, glob_pattern: str = 'mp_rank_*_model_states.pt', model_parallel: int = 8, param_list: List[Param] = EVO2_PARAMS, exclude_extra_params: bool = False, verbose: bool = False):
    ...
    check_params(detected=model_parameter_names, expected=set(param.name for param in param_list), buffers=buffers, verbose=verbose)
    # Convert the checkpoint
    convert(input_data_shards, output_data_shards, model_parameter_names, param_list, verbose=verbose, exclude_extra=exclude_extra_params)
    logging.info("Done!")
